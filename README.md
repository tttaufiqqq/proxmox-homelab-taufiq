...
| 104 | linux-mysql | Ubuntu 24.04 | 192.168.0.103 | 100.115.237.93 | MySQL 8.0 — also backs [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `shelter` connection (database `workshop_2_prod`) — see [`docs/12-mysql-shelter-animals-split/`](docs/12-mysql-shelter-animals-split/mysql-shelter-animals-split.md) |
| 105 | linux-mariadb | Ubuntu 24.04 | 192.168.0.105 | 100.78.124.25 | MariaDB 10.11 — also backs [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `reporting` connection (database `workshop_2_prod`) — `booking` split off onto CT 113 on 2026-07-20, see [`docs/13-mariadb-reporting-booking-split/`](docs/13-mariadb-reporting-booking-split/mariadb-reporting-booking-split.md) |
| 106 | linux-postgres | Ubuntu 24.04 | 192.168.0.107 | 100.113.234.24 | PostgreSQL 16 — also backs [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `users` connection (database `workshop_2_prod`) |
...
| 111 | linux-gh-runner | Ubuntu 24.04 (unprivileged) | 192.168.0.111 | 100.72.6.40 | Self-hosted GitHub Actions runner for [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop) CI **and** CD — added 2026-07-19, reads DB creds from `linux-vault` via a scoped read-only token (not the root token); also runs Ansible against all 6 fleet hosts on a dedicated CD SSH key |
| 112 | linux-mysql-2 | Ubuntu 24.04 (unprivileged) | 192.168.0.112 | 100.123.221.89 | MySQL 8.0 — backs [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `animals` connection (database `workshop_2_prod`) — added 2026-07-20 as a CT rather than a VM (same persistent-workload reasoning as `linux-mongodb`/`linux-vault`), see [`docs/12-mysql-shelter-animals-split/`](docs/12-mysql-shelter-animals-split/mysql-shelter-animals-split.md) |
| 113 | linux-mariadb-2 | Ubuntu 24.04 (unprivileged) | 192.168.0.113 | 100.97.35.29 | MariaDB 10.11 — backs [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `booking` connection (database `workshop_2_prod`) — added 2026-07-20, same day as CT 112, same CT-over-VM reasoning — see [`docs/13-mariadb-reporting-booking-split/`](docs/13-mariadb-reporting-booking-split/mariadb-reporting-booking-split.md) |
...
| 10 | [`docs/09-github-actions-runner/actions-runner-setup.md`](docs/09-github-actions-runner/actions-runner-setup.md) | Self-hosted GitHub Actions runner for [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop), run as an LXC container on the tailnet instead of a GitHub-hosted runner so it can reach the app's private DB servers; scoped read-only Vault token (not the root token used everywhere else) so a workflow can only read the three DB secrets it needs. Later extended to carry CD as well — Ansible on the runner converging all 6 fleet hosts, a dedicated `gh-runner-cd` SSH key that's revocable independently of the human admin key, a `secret/asw-cd` Vault path, and UFW rules opening each host to this CT |
...
| 15 | [`docs/14-laravel-log-permission-denied/laravel-log-permission-denied.md`](docs/14-laravel-log-permission-denied/laravel-log-permission-denied.md) | [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `app-server` had its Laravel log file end up owned by the wrong group after a manual `artisan` command ran outside the web server's user, taking booking confirmation and animal-matching down with cascading "permission denied" errors; root cause was a missing setgid bit on `storage/`, fixed on the host and made permanent in the deploy playbook |
| 16 | [`docs/15-app-server-backups-permission-denied/app-server-backups-permission-denied.md`](docs/15-app-server-backups-permission-denied/app-server-backups-permission-denied.md) | [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop)'s `/admin/backups` 500ing turned out to be two intentional decisions fighting each other — a deliberately locked-down `0700` backups directory vs. a documented admin feature that needs to read it — resolved by opening the tree to `www-data`, reasoned through against what that boundary actually protects given `www-data` already holds live DB credentials |
| 17 | [`docs/16-stored-routine-definer-privilege-split/stored-routine-definer-privilege-split.md`](docs/16-stored-routine-definer-privilege-split/stored-routine-definer-privilege-split.md) | A user-reported "failed to delete report" in [`Animal-Shelter-Workshop`](https://github.com/tttaufiqqq/Animal-Shelter-Workshop) turned out to be a full production outage of the entire stored-procedure layer on all 4 MySQL/MariaDB hosts (121 objects) — every routine's `DEFINER` was still the old shared `workshop_2` credential from before the prod/dev split, which has zero privileges on the new `workshop_2_prod` database, so `SQL SECURITY DEFINER` denied even `root`; fixed with `SQL SECURITY INVOKER` for procedures/functions and a `DEFINER`-corrected `DROP`+`CREATE` for triggers (no `ALTER` equivalent exists) |
| 18 | [`docs/17-custom-ssh-motd/`](docs/17-custom-ssh-motd/00-why-visual-host-identity.md) | Two docs on making every host visually identifiable over SSH: a per-role, per-host-colored dynamic login banner (figlet role banner, live stats, service health check) across all 13 original VMs/CTs, plus a fastfetch splash for the `taufiq` host node; and a matching rounded-pill oh-my-posh prompt using the same per-host colors, extended to `linux-k3s`, `linux-observability`, and `taufiq` too. Found and fixed along the way: a `linux-vault` DNS gap blocking `apt`, a `VAULT_ADDR`/color bug from how `pam_motd` invokes scripts, three hosts whose real OS hostname silently didn't match the DNS alias (`workshop-2`/`workshop-mysql`/`workshop-postgres`), `linux-oracle-db` needing an `/etc/profile.d/` equivalent and, later, a Tailscale-borrowed binary to route around its DNS gap, and a stuck `tailscaled` process blocking SSH entirely on `linux-mongodb` |
| 19 | [`docs/18-network-segmentation/network-segmentation-execution.md`](docs/18-network-segmentation/network-segmentation-execution.md) | Turning [`homelab-network-segmentation-execution-plan.md`](plans/01-homelab-network-segmentation-execution-plan%20(executed).md) into a running network — OPNsense router VM, per-VLAN Kea DHCP, firewall rules across 8 VLANs; found and fixed along the way: a silently truncated installer ISO (verified against OPNsense's published checksum), a firewall rule design that accidentally blocked OPNsense's own DNS resolver, Oracle Linux's `firewalld` and Ubuntu's `ufw` both independently gating traffic regardless of any VLAN rule |
| 20 | [`docs/19-devops-practice/`](docs/19-devops-practice/00-why-devops.md) | A 12-part series turning [`devops-practice-plan.md`](plans/02-devops-practice-plan%20(executed).md) into practice: Terraform (proving the loop, then state/import/module, then importing the rest of the live fleet), Ansible (roles, idempotency, Molecule), Docker, k3s + Vault injector (later extended into a real 2-node cluster, full CI→ArgoCD image-build automation, and `cloudflared` migrated fully into the cluster via [`plans/06-k3s-multi-node-gitops-automation-plan.md`](plans/06-k3s-multi-node-gitops-automation-plan%20(executed).md)), CI/CD gaps, observability (Prometheus/Grafana/Loki/Alertmanager), GitOps (ArgoCD), and an Azure stretch (offsite backup sync, a Function reading Vault through Tailscale Funnel, Terraform against a second provider). Each doc is what I built, why, what broke, how I found it, how I recovered, same as every doc above |
| 21 | [`docs/20-homelab-terraform/homelab-terraform-split.md`](docs/20-homelab-terraform/homelab-terraform-split.md) | Splitting Terraform ownership: `Animal-Shelter-Workshop`'s own Terraform is scoped back down to just what that app needs (5 DB connections, `app-server`, `linux-vault`, `linux-gh-runner`), and the genuinely shared homelab infrastructure (`linux-mini-io`, `linux-k3s`, `linux-mongodb`, `linux-observability`) gets its own Terraform config in this repo instead — the first real infrastructure code (not just docs) to live here |
