...
| 19 | [`docs/18-network-segmentation/network-segmentation-execution.md`](docs/18-network-segmentation/network-segmentation-execution.md) | Turning [`homelab-network-segmentation-execution-plan.md`](plans/homelab-network-segmentation-execution-plan.md) into a running network — OPNsense router VM, per-VLAN Kea DHCP, firewall rules across 8 VLANs; found and fixed along the way: a silently truncated installer ISO (verified against OPNsense's published checksum), a firewall rule design that accidentally blocked OPNsense's own DNS resolver, Oracle Linux's `firewalld` and Ubuntu's `ufw` both independently gating traffic regardless of any VLAN rule |
| 20 | [`docs/19-devops-practice/`](docs/19-devops-practice/00-why-devops.md) | A 12-part series turning [`devops-practice-plan.md`](plans/devops-practice-plan.md) into practice: Terraform (proving the loop, then state/import/module, then importing the rest of the live fleet), Ansible (roles, idempotency, Molecule), CI/CD gaps, Docker, k3s + Vault injector, observability (Prometheus/Grafana/Loki/Alertmanager), GitOps (ArgoCD), and an Azure stretch (offsite backup sync, a Function reading Vault through Tailscale Funnel, Terraform against a second provider). Each doc is what I built, why, what broke, how I found it, how I recovered, same as every doc above |
