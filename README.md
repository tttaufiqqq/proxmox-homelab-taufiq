...
| 18 | [`docs/17-custom-ssh-motd/custom-ssh-motd-setup.md`](docs/17-custom-ssh-motd/custom-ssh-motd-setup.md) | A per-role, per-host-colored dynamic SSH login banner rolled out to all 13 VMs/CTs in the fleet — figlet role banner, live stats, service health check; found and fixed along the way: a `linux-vault` DNS gap blocking `apt`, a `VAULT_ADDR`/color bug from how `pam_motd` invokes scripts, three hosts whose real OS hostname silently didn't match the DNS alias (`workshop-2`/`workshop-mysql`/`workshop-postgres`), and `linux-oracle-db` needing an `/etc/profile.d/` equivalent since Oracle Linux has no `update-motd.d` mechanism at all |
